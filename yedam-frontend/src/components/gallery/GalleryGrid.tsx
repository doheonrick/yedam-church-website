    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
      {items.map((item, idx) => (
        <button
          ...
          className="group relative aspect-square overflow-hidden rounded-md bg-warm-stone focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"
        >
          <img
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-3 text-left opacity-0 group-hover:opacity-100 transition-opacity">
            <p className="text-xs text-brand-gold-light">{item.category}</p>
